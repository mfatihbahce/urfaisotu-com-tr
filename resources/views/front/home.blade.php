...
@section('content')
    <section class="bg-gradient-to-r from-primary-dark to-primary rounded-2xl p-8 md:p-12 text-white mb-12">
        ...
        <a href="{{ route('products.index') }}" class="inline-block px-6 py-3 bg-white text-primary-dark font-semibold rounded-lg hover:bg-slate-100">Ürünleri Keşfet</a>
    </section>
        ...
        <section class="mb-12">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="hidden md:flex items-center gap-2">
                    <button type="button" id="featured-prev" aria-label="Sol" class="w-10 h-10 rounded-full bg-white shadow border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-primary hover:text-white hover:border-primary transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    </button>
                    <button type="button" id="featured-next" aria-label="Sağ" class="w-10 h-10 rounded-full bg-white shadow border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-primary hover:text-white hover:border-primary transition-all">
    ...
    @if($categories->isNotEmpty())
        <section class="mb-12">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="hidden md:flex items-center gap-2">
                    <button type="button" id="categories-prev" aria-label="Sol" class="w-10 h-10 rounded-full bg-white shadow border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-primary hover:text-white hover:border-primary transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                    </button>
                    <button type="button" id="categories-next" aria-label="Sağ" class="w-10 h-10 rounded-full bg-white shadow border border-slate-200 flex items-center justify-center text-slate-600 hover:bg-primary hover:text-white hover:border-primary transition-all">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                    </button>
                </div>
            </div>
            <div class="relative -mx-4 sm:-mx-6 px-4 sm:px-6">
                <div id="categories-carousel" class="flex overflow-x-auto gap-4 pb-2 snap-x snap-mandatory scroll-smooth" style="scrollbar-width: none; -ms-overflow-style: none;">
                    @foreach($categories as $cat)
                        <a href="{{ route('categories.show', $cat->slug) }}" class="flex-shrink-0 w-[calc(20%-1rem)] min-w-[130px] sm:min-w-[150px] lg:min-w-[180px] snap-start bg-white rounded-xl shadow-sm p-5 sm:p-6 text-center hover:shadow-md hover:border-primary transition border-2 border-transparent">
    ...
    <section>
        ...
        <div class="mt-8 text-center">
            <a href="{{ route('products.index') }}" class="inline-flex items-center gap-2 px-6 py-3 bg-primary-dark text-white font-semibold rounded-xl hover:bg-primary transition-colors">
                Tüm Ürünleri Gör
