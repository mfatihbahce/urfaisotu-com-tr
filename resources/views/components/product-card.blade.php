...
<article class="group bg-white rounded-2xl shadow-sm hover:shadow-xl transition-all duration-300 overflow-hidden border border-slate-100 hover:-translate-y-1 flex flex-col">
    <a href="{{ route('products.show', $product->slug) }}" class="block flex-1 flex flex-col">
        ...
        <div class="relative aspect-square bg-cream overflow-hidden">
            ...
            <div class="absolute top-3 right-3 flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <a href="{{ route('products.show', $product->slug) }}" title="Hızlı Görüntüle" class="w-9 h-9 bg-white/95 rounded-full flex items-center justify-center text-slate-700 shadow-md hover:bg-primary hover:text-white transition-colors">
        ...
        <div class="p-4 flex-1 flex flex-col">
            @if($product->category)
                <p class="text-xs text-primary-dark font-medium uppercase tracking-wide mb-1">{{ $product->category->name }}</p>
            @endif
            <h3 class="font-semibold text-slate-800 group-hover:text-primary-dark transition-colors line-clamp-2 min-h-[2.5rem]">{{ $product->name }}</h3>
            ...
            <div class="mt-3 flex items-center gap-2 flex-wrap">
                <span class="text-lg font-bold text-primary-dark">{{ number_format($product->effective_price, 2, ',', '.') }} ₺</span>
    ...
    <div class="p-4 pt-0">
        <form action="{{ route('cart.add') }}" method="post" class="mt-0">
            ...
            @endif
            <button type="submit" class="w-full py-3 px-4 bg-primary-dark text-white text-center font-medium rounded-xl hover:bg-primary transition-colors duration-200">
                Sepete Ekle
            </button>
        </form>
    </div>
